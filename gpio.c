...
{
	if(handler_led != NULL)
		*((uint16_t*)(handler_led->_register)) |= (handler_led->value) << (handler_led->pin);
...
{
	handler_joy->input.valueUp     = (READ(handler_joy->pin.up)) 	 == HIGH ? IS_PRESSED : !IS_PRESSED;
	handler_joy->input.valueDown   = (READ(handler_joy->pin.down))   == HIGH ? IS_PRESSED : !IS_PRESSED;
	handler_joy->input.valueLeft   = (READ(handler_joy->pin.left))   == HIGH ? IS_PRESSED : !IS_PRESSED;
	handler_joy->input.valueRight  = (READ(handler_joy->pin.right))  == HIGH ? IS_PRESSED : !IS_PRESSED;
	handler_joy->input.valueCenter = (READ(handler_joy->pin.center)) == HIGH ? IS_PRESSED : !IS_PRESSED;
}
...
static void gpio_writeConfig				 	(GPIO_HANDLER_JOYSTICK_t *handler_joy)
{
	...
	*((uint16_t*)(handler_joy->_register)) |= ( ( (handler_joy->bit.up)     << (handler_joy->pin.up     * 2) )|
...
void 		gpio_initJoystick    (GPIO_HANDLER_JOYSTICK_t *handler_joy)
{
	...
	gpio_selectPulldownResistorsJoystick(handler_joy);				// Set pulldown resistors
	gpio_writeConfig   					(handler_joy);
